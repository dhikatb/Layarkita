    ...
    public void onClick(View v) {
       ...
       final String cekDesk = etDeskripsi.getText().toString();
        if (cekJudul.length() < 5) {
            ...
            PostResponseAsyncTask taskInsert = new PostResponseAsyncTask(InsertActivity.this, postData,
                    new AsyncResponse() {
                        @Override
                        public void processFinish(String s) {
                            Log.d(LOG, s);
                            if (s.contains("success")) {
                                Toast.makeText(InsertActivity.this, "Video Berhasil Dimasukan!", Toast.LENGTH_LONG).show();
                                Intent in = new Intent(InsertActivity.this, ListActivity.class);
                                startActivity(in);
                            } else {
                                Toast.makeText(InsertActivity.this, "Form Harus diisi!", Toast.LENGTH_SHORT).show();
                            }
                        }
                    });
            taskInsert.execute("http://192.168.201.1/layarkita/insert.php");
        }
    }
}
